...
- [ ] T010 コンテナ化用 Dockerfile と CI/CD ビルドパイプラインを整備
- [ ] T011 ECS タスク定義・サービス（Fargate）とヘルスチェック/オートスケール設定を作成
- [ ] T012 Lambda 用ビルド（`cargo lambda build`）とデプロイパッケージ/設定を整備
...
- [ ] T020 [P] [US1] 契約テスト `tests/contract/` を追加
- [ ] T021 [P] [US1] 結合テスト `tests/integration/` を追加

### User Story 1 実装

- [ ] T022 [P] [US1] ドメインモデルを `src/models/` に追加
- [ ] T023 [P] [US1] アプリケーションサービスを `src/services/` に実装
- [ ] T024 [US1] GraphQL Mutation や API エンドポイントを実装
- [ ] T025 [US1] バリデーションとエラーハンドリングを追加
- [ ] T026 [US1] ロギング・監査情報を追加
...
- [ ] T032 [P] [US2] モデル／投影を追加
- [ ] T033 [US2] サービス・ユースケース層を実装
- [ ] T034 [US2] GraphQL Query / Subscription を更新
- [ ] T035 [US2] User Story 1 との連携を調整（必要な場合）
...
### MVP 優先
 ...
 2. User Story 1 (P1) を実装し、GraphQL/BFF 経由かつ ECS サービスで独立してテスト・デモ可能にする
3. 必要に応じてデプロイ／レビュー

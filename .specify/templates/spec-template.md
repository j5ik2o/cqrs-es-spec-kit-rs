...
- **ドメインイベント**: [`references/cqrs-es-example-rs/modules/command/domain/src/*/*_events.rs` に倣い、イベント型を別ファイルで定義し、バージョンやシリアライズ形式を明記する]
...
- 回復可能なアプリケーションエラーを `Result` 等で返却する設計を明示し、ユースケース単位でエラー型を定義する。

### コマンド & クエリモデル

- **コマンド一覧**: [集約の公開メソッドとして実装するコマンドを列挙し（例: `group_chat.rs`）、入力、期待するイベント、検証ルールを記載する。独立したコマンドクラスを作成してはならない。イベント保存が主目的である一方、リプレイや他集約確認など必要な読み込みの範囲も明記]
...
- **ライブラリ設定**: [`j5ik2o/event-store-adapter-rs` の接続情報、使用するストレージバックエンド]
- **スナップショット/リプレイ**: [スナップショット戦略、イベント再生手順、障害復旧手順]
- **監視**: [イベント保存失敗・ストリーム競合の検出と通知方法]
- **参考実装**: [`references/event-store-adapter-rs` の参照ファイル/設定を列挙し、差分と理由を記載（コードコピー不可、設計意図のみ反映）]
...
- **イベントストア設定**: [DynamoDB と `j5ik2o/event-store-adapter-rs` の連携、スナップショットとイベントストリームの扱い]
...
- **参考リポジトリ**: [`references/cqrs-es-example-rs` の該当箇所と本仕様の差分/適用箇所]
- **テーブルフォーマット**: [`references/cqrs-es-example-rs/tools/dynamodb-setup/create-tables.sh` に基づくジャーナル/スナップショットテーブル形式、導入する差異と移行手順]

### リポジトリ構成と依存制御

- **ワークスペース設定**: [pnpm workspace の設定ファイル、`modules/*` のパターン、turbo のタスク構成]
- **パッケージ一覧**: [`references/cqrs-es-example-rs/modules` をもとに、本仕様で追加/調整するサブプロジェクト一覧とクリーンアーキテクチャ層との対応]
- **依存方向**: [各パッケージの `dependencies`/`dev-dependencies` ポリシー、許可される依存方向、禁止例、静的チェック方法]
...
- **リファレンス**: [`references/cqrs-es-example-rs` の利用箇所（コマンド、クエリ、投影など）と、本仕様との差分・適用理由。参照のみでコードコピーは行わないことを明示]

...
バージョン: 1.2.0 → 1.3.0
更新した原則:
- DDD集約境界の厳守 → DDD集約境界の厳守（ECS と Lambda の役割分離を明記）
- イベントストア標準化とDynamoDB運用 → イベントストア標準化とDynamoDB運用（イベントフォワーダー/リードモデル更新を Lambda 運用に戻し、他アプリを ECS 想定と定義）
- CQRSとGraphQL契約の一貫性 → CQRSとGraphQL契約の一貫性（ECS 上のアプリ構成を前提化）
- ローカルサンドボックス整合性 → ローカルサンドボックス整合性（Lambda 手順を再確認し、ECS 模擬の言及を整理）
- OpenSpecドキュメント同期と品質ゲート → OpenSpecドキュメント同期と品質ゲート（Lambda/ECS の混在構成の検証を追加）
追加セクション:
- 技術・インフラ要件（Lambda と ECS の責務分離を反映）
削除セクション: なし
テンプレート更新状況:
...
- MUST サービス分離を守り、ECS 上のアプリケーションサービスでも Lambda 上のイベント処理でもコマンドはドメインイベントへ変換し、副作用はユースケース層で制御する。
...
### CQRSとGraphQL契約の一貫性
- MUST コマンドサービス、GraphQL BFF、Next.js フロントエンドを独立デプロイし、それぞれ ECS サービスとして運用しても BFF がコマンドのビジネスロジックを保持しない構造を維持する。
...
### ローカルサンドボックス整合性
- MUST `docker-compose.local.yml` で LocalStack・MySQL・Redis を起動し、Lambda と ECS 本番構成の差異をドキュメント化して補正する。
- MUST `scripts/localstack/bootstrap.sh` と `scripts/localstack/deploy-lambdas.sh` を用いてリソース初期化と Lambda デプロイを行い、CIでも同手順で検証する。
- SHOULD BFF や Next.js など ECS 運用予定のアプリはローカルで Docker もしくはホスト実行し、差分があれば docs/local-sandbox.md に記す。
...
- MUST ECS アプリケーションと Lambda イベント処理の両方についてテスト戦略（ユニット／契約／E2E）を定義し、CI に組み込む。
理由: ドキュメント同期と品質ゲートを強制することで、仕様逸脱と実装ドリフトを未然に防ぐ。

## 技術・インフラ要件

本プロジェクトは DynamoDB をイベントストア、Kinesis をストリーム処理、MySQL をリードモデルとして運用し、アプリケーションの実行基盤は ECS（Fargate 推奨）と Lambda を併用する。以下を遵守すること。

- **アーキテクチャ境界**
  - コマンドアプリケーションサービス（Rust）、GraphQL BFF、Next.js フロントエンドはそれぞれ独立した ECS サービス/デプロイ単位とし、サービス間通信越しにユースケースを呼び出す。
...
- **データストアとリードモデル**
  ...
  - リードモデル更新 Lambda はイベント ID の処理履歴で冪等性を保証し、再投影・リプレイで重複反映を防止する。
  - ECS サービスから MySQL への接続は必ず RDS Proxy もしくはコネクションプール経由とし、ローカル環境では `mysql:8.0` コンテナを利用する。
- **BFF とフロントエンド（ECS 運用）**
...
- **Lambda 運用基準（イベントフォワーダー/リードモデル更新）**
  ...
  - リードモデル更新サービスは Kinesis イベントから MySQL を更新し、`mysql_async` または `sqlx` を用いて RDS Proxy 経由で接続し、処理済みイベントを追跡・記録する。
  - すべての Lambda は構造化ログ（`tracing`）を出力し、CloudWatch Logs や LocalStack Logs で監視できる状態を維持する。
- **ECS 運用基準（アプリケーションサービス/BFF/Next.js）**
  - コンテナイメージは CI でビルドし、セキュリティスキャンとタグ付け後にリポジトリへ登録する。
  - 本番は ECS Fargate（もしくは適切な起動タイプ）で稼働させ、ローリングデプロイとヘルスチェックを設定する。
...
- ローカルおよび CI で `cargo test`, コンテナビルド, LocalStack での E2E（Lambda）と ECS アプリの統合テストを実施し、失敗を許容しない。
...
**Version**: 1.3.0 | **Ratified**: 2025-11-01 | **Last Amended**: 2025-11-01
